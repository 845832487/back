...
public class StudentDetailController {
    ...
    public Result<?> delMultiStudents(@RequestBody String studentDetails) {
        try {
            ...
            for (int i = 0; i < detailArray.size(); i++) {
                ...
                int roomsLeft = dormDetailMapper.selRoomsLeftById(
                        detailArray.getJSONObject(i).getString("buildingNum"),
                        detailArray.getJSONObject(i).getString("dormNum"));
                dormDetailMapper.setRoomsLeftById(detailArray.getJSONObject(i).getString("buildingNum"),
                        detailArray.getJSONObject(i).getString("dormNum"),
                        roomsLeft + 1);
            }
    ...
    public Result<?> delSingleStudent(String studentId) {
        try {
            StudentDetail student = studentDetailMapper.selectById(studentId);
            studentDetailMapper.deleteById(studentId);
            userLoginMapper.deleteById(studentId);
            Integer roomsLeft = dormDetailMapper.selRoomsLeftById(student.getBuildingNum(), student.getDormNum());
            dormDetailMapper.setRoomsLeftById(student.getBuildingNum(), student.getDormNum(), roomsLeft + 1);
        } catch (Exception e) {
